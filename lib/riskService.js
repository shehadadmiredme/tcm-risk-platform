...
function extractDose(line, allowNonWeight) {
  const s = String(line);
  ...
  // 非重量单位（枚/片/丸等）：仅当处于明确剂量区（冒号后）时匹配，
  // 避免把药材名里的字（如「一枝黄花」的「枝」）误当剂量。
  if (allowNonWeight) {
...
function parseHerbs(text) {
  return String(text || '')
    .split(/[\n,，、;；]+/)
    .map(function (line) {
      // 冒号后为明确剂量区（允许非重量单位，如「白术：四十枚」）；
      // 无冒号时只匹配重量单位剂量，避免把药材名里的字（如「一枝黄花」的「枝」）误当剂量。
      const colonMatch = line.match(/^([^：:]*?)\s*[：:]\s*(.+)$/);
      let namePart = line;
      let dose = null;
      if (colonMatch) {
        namePart = colonMatch[1];
        dose = extractDose(colonMatch[2], true);
      } else {
        dose = extractDose(line, false);
        if (dose) namePart = namePart.replace(dose.text, '');
      }
