...
function convertDoseText(text) {
  ...
  const range = t.match(/(\d+(?:\.\d+)?)\s*[〜～~\-–至]\s*(\d+(?:\.\d+)?)\s*(克|g|G|两|钱|分|斤)/);
  if (range && UNIT_TO_G[range[3]]) {
    return (Number(range[1]) + Number(range[2])) / 2 * UNIT_TO_G[range[3]];
  }
  const arabicHalf = t.match(/(\d+(?:\.\d+)?)\s*(克|g|G|两|钱|分|斤)(半)/);
  if (arabicHalf && UNIT_TO_G[arabicHalf[2]]) {
    return Number(arabicHalf[1]) * UNIT_TO_G[arabicHalf[2]] + UNIT_TO_G[arabicHalf[2]] * 0.5;
  }
  const chineseHalf = t.match(/([一二两三四五六七八九十百千万]+)\s*(克|g|G|两|钱|分|斤)(半)/);
  if (chineseHalf && UNIT_TO_G[chineseHalf[2]]) {
    const n = chineseToNumber(chineseHalf[1]);
    return n === null ? null : n * UNIT_TO_G[chineseHalf[2]] + UNIT_TO_G[chineseHalf[2]] * 0.5;
  }
  const single = t.match(/(\d+(?:\.\d+)?)\s*(克|g|G|两|钱|分|斤)/);
  if (single && UNIT_TO_G[single[2]]) return Number(single[1]) * UNIT_TO_G[single[2]];
  const chinese = t.match(/([一二两三四五六七八九十百千万]+)\s*(克|g|G|两|钱|分|斤)/);
  if (chinese && UNIT_TO_G[chinese[2]]) {
    const n = chineseToNumber(chinese[1]);
    return n === null ? null : n * UNIT_TO_G[chinese[2]];
  }
  return null;
}
...
function extractDose(line) {
  const s = String(line);
  const arabicHalf = s.match(/(\d+(?:\.\d+)?)\s*(克|g|G|两|钱|分|斤)(半)/);
  if (arabicHalf) return { text: arabicHalf[0], valueG: convertDoseText(arabicHalf[0]) };
  const chineseHalf = s.match(/([一二两三四五六七八九十百千万]+)\s*(两|钱|分|斤)(半)/);
  if (chineseHalf) return { text: chineseHalf[0], valueG: convertDoseText(chineseHalf[0]) };
  const arabic = s.match(/(\d+(?:\.\d+)?)\s*(克|g|G|两|钱|分|斤|枚|个|片|粒|丸|锭|团|束|把|根|枝|条|块|段|只|头|对|付|帖|剂|盏|杯|碗|匙|捻|撮|挺|寸|尺|丈|层|钟|盂|桶|驮|擘|服|具|脚|株|张|页|尾|掬)/);
  if (arabic) return { text: arabic[0], valueG: convertDoseText(arabic[0]) };
  const chinese = s.match(/([一二两三四五六七八九十百千万]+)\s*(两|钱|分|斤|枚|个|片|粒|丸|锭|团|束|把|根|枝|条|块|段|只|头|对|付|帖|剂|盏|杯|碗|匙|捻|撮|挺|寸|尺|丈|层|钟|盂|桶|驮|擘|服|具|脚|株|张|页|尾|掬)/);
  if (chinese) return { text: chinese[0], valueG: convertDoseText(chinese[0]) };
  return null;
}
